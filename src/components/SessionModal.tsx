...
export function SessionModal({
  ...
  const selectedClass = groupClasses.find((c) => c.id === classId);

  // Show student names for selected group class
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectionCount > 1) {
      toast.error('Please select only one of student, group class, or guest');
      return;
    }
    ...
    const finalRateValue = rateMode === 'auto' ? null : parseFloat(rateValue) || 0;
        ...
        const movedFromDate =
      session && date !== session.plannedDate
        ? session.movedFromDate ?? session.plannedDate
        : session?.movedFromDate || null;
    const movedFromTime =
      session && time !== session.plannedTime
        ? session.movedFromTime ?? session.plannedTime
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
            >
              <option value="">{groupClasses.length > 0 ? 'Select a group class' : 'No group classes'}</option>
